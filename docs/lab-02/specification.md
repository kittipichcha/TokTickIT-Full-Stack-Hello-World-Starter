...
- **BR-01** The official Ticket Number is generated by the backend and must be unique. Format: `TKT-{YYYY}-{6-digit sequence}` (e.g., `TKT-2026-000123`), sequence per calendar year. Boundary assertions: generated number must match the pattern, be unique across all tickets, and reuse the same format; tests must verify this explicitly rather than just checking for "a number".
