...
| API-ATT-14-PDF | Integration | PDF preview never returns application/pdf (real DB) | Uploads a real two-page PDF fixture, previews it, and asserts the response is never `application/pdf`. If rendering succeeds, verifies `200 image/png` with PNG magic bytes. If rendering fails (e.g., sharp lacks PDF support), asserts `500 INTERNAL_ERROR`. A second test uploads a PDF with zero pages and asserts the response is never `application/pdf`. | `server/tests/lab-02/attachment-concurrency.integration.test.ts` | FR-12 | BR-28 | AC-24 | **Passed** |
...
| API-ATT-06 | API | BR-17 partial success: ticket POST succeeds, attachment upload fails, ticket persists and is not rolled back | The ticket remains persisted after attachment failure and no duplicate ticket is created. | `server/tests/lab-02/attachments.api.test.ts` | FR-02, FR-10 | BR-17 | AC-26 | **Passed** |
